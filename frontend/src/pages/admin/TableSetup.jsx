...
const SECTIONS = ['indoor', 'outdoor', 'terrace', 'private'];
...
function TableModal({ table, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.number?.[0] || 'Failed to save table');
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-title">{table?.id ? 'Edit' : 'Add'} Table</div>
          <button className="modal-close" onClick={onClose}>×</button>
        </div>
        <div className="grid-2" style={{ gap: 12 }}>
          <div className="form-group">
            <label className="form-label">Table Number *</label>
            <input className="form-input" value={form.number} onChange={e => setForm(f => ({ ...f, number: e.target.value }))} placeholder="T1, T2..." />
          </div>
          <div className="form-group">
            <label className="form-label">Display Name *</label>
            <input className="form-input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Table 1, Terrace A..." />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={form.section} onChange={e => setForm(f => ({ ...f, section: e.target.value }))}>
              {SECTIONS.map(s => <option key={s} value={s}>{SECTION_ICONS[s]} {s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
            </select>
          </div>
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, margin: '8px 0 16px' }}>
...
export default function TableSetup() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await tableApi.deleteTable(id); toast.success('Table deleted'); fetchTables(); }
    ...
    <div className="fade-in">
      ...
      <div className="category-tabs mb-4">
        {sections.map(s => (
          ...
          >
            {s === 'all' ? '🪑 All Tables' : `${SECTION_ICONS[s]} ${s.charAt(0).toUpperCase() + s.slice(1)}`}
          </button>
        ))}
      </div>
            ...
            <div key={table.id} className="card card-hover">
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <span className={`badge badge-muted`} style={{ alignSelf: 'flex-start' }}>
                  {SECTION_ICONS[table.section]} {table.section}
                </span>
              </div>
